...
export const generateCode = (chars, banks, addComments, setCode) => {
    ...
    const charDefinitions = chars.map(char => {
        const sanitizedName = sanitizeIdentifier(char.name, 'unnamed_char');

        const bytes = char.pixels.map(row => {
            // Convert a 5-bit row into its numeric value.
            return row.reduce((acc, pixel, idx) => {
                return acc | (pixel ? 1 << (4 - idx) : 0);
            }, 0);
        });
    ...
    // Generate code for character banks
    let bankCode = '';
    if (banks.length > 0) {
        ...
        banks.forEach((bank, bankIndex) => {
            ...
            const bankChars = bank.characters
                .map((charIndex, index) => {
                    const charName = chars[charIndex]
                        ? sanitizeIdentifier(chars[charIndex].name, `char_${charIndex}`)
                        : 'NULL';
                    const hexIndex = index.toString(16).toUpperCase();
                    return {
                        name: charName,
                        hexIndex: `0x${hexIndex.padStart(2, '0')}`,
                        index: index
                    };
                })
                .concat(Array(8).fill({ name: 'NULL', hexIndex: '0x00', index: -1 }))
                .slice(0, 8)
                .map((char, index) => {
                    if (addComments) {
            ...
            const enumValues = bank.characters
                .map((charIndex, idx) => {
                    ...
                    if (!char) return null;
                    const sanitizedName = sanitizeIdentifier(char.name, `char_${charIndex}`);
                    if(addComments){
